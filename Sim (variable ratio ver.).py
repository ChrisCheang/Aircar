...
def print_stuff_turbine(ratio):
    print(round(clock, 1), "s,",
          round(turbine.p0, 1), "Pa (inlet), ",
          round(turbine.max_power(), 2), "W max, ",
          round(car.rpm * ratio, 0), "rpm operation, ",
          round(turbine.no_load_rpm(), 0), "rpm max, ",
          round(turbine.torque(car.rpm * ratio), 3), "Nm operation, ",
          turbine.specific_speed(car.rpm * ratio * 2 * np.pi / 60), "specific speed"
          )

gear_ratios = np.linspace(3, 10, 50)
finish_times = []
...
for ratio in gear_ratios:
    ...
    turbine = TurbineDrive(p0=nozzle.pb, T0=nozzle.T0, d0=nozzle.d0, p1=p_ambient, T1=298, d1=1,
                           nozzle=nozzle)  # the d0 is wrong but not used so ignore for now
    ...
    car = Aircar(s=0, v=0, a=0, m=3.0, rpm=0, t=turbine.torque(0) * ratio * 0.85, drive=turbine, step_down=ratio)
    ...
    while not end:
        car = car.update(time_step, turbine, ratio)
...
plot.plot(gear_ratios, finish_times)
plot.title('Finishing times (s) at different gear ratios')
plot.xlabel('Gear ratio')
plot.ylabel('Finish time (s)')
plot.grid(which='major', alpha=0.2)
